...
const onDroppedImage = (pictureArr, setHomepageImage) => {
    // pull off file from array (MAX one-1 item)
    const selectedFile = pictureArr[0];
    ...
    axios.post(`${process.env.REACT_APP_BASE_URL}/upload/misc/file`, data, config).then((res) => {
        if (res.data.message === "Successfully uploaded file!") {
            ...
            // update current file in main component
            NotificationManager.success(`We've successfully uploaded your file! Please proceed filling out your information or add more files.`, 'Successfully uploaded file!', 4500);
        } else if (res.data.message === "Could not detect a proper file - please upload another file or contact support if this issue persists!") {
            NotificationManager.error(res.data.message, 'ERROR UPLOADING FILE!', 4500);
        } else {
            console.log("Err", res.data);
        }
    }).catch((err) => {
