...
router.post("/", (req, resppppp, next) => {
    ...
    collection.findOne({ id: blogID }).then((blog) => {
        if (!blog) {
            ...
            const foundIndex = blog.likedBy.findIndex((item) => item.reactorID === signedinUserID);

            if (foundIndex !== -1) {
                blog.likedBy.splice(foundIndex, 1);
                blog.likes -= 1;
                ...
                collection.save(blog, (error, result) => {
                    if (error) {
                        ...
                        resppppp.json({
                            message: "Successfully 'liked' this blog post!",
                            blog
                        })
                    }
                })
                ...
                blog.likes += 1;
                blog.likedBy.push(newLikedByObj);
