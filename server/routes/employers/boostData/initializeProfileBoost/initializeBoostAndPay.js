...
const createNewBoostedProfileSchema = require("../../../../schemas/boostedData/employerProfile/boostEmployerProfileSchema.js");
...
router.post("/", async (req, resppppp, next) => {
    ...
    const alreadyBoostedCollection = Connection.db.db("db").collection("boostedemployerprofiles");
    const secondsInDay = 86400;
    
    const foundEmployerAccount = await employerCollection.findOne({ uniqueId: employerId });
    const alreadyBoostedOrNot = await alreadyBoostedCollection.findOne({ uniqueId: employerId });

    console.log("-----------------------------------------------------------", alreadyBoostedOrNot);

    const calculateTierSecondsTTL = () => {
        switch (profileBoostedSelected) {
            case "tier-1":
    ...
    const { 
        ...
        profilePicsVideos,
        profileBannerImage
    } = foundEmployerAccount;

    const newUserData = createNewBoostedProfileSchema(calculateTierSecondsTTL(), {
        firstName, 
        lastName, 
        email, 
        currentlyFollowedBy,
    ...
    if (alreadyBoostedOrNot === null) {
                ...
                if (err) {
                    ...
                    newUserData.save((err, result) => {
                        if (err) {
                            ...
                            resppppp.json({
                                message: "Successfully boosted your selected profile!",
                                user: foundEmployerAccount,
                                cost: calculateCost()
                            })
                        }
                    })
        ...
        resppppp.json({
            message: "Already boosted this profile account!"
        })
    }
});
