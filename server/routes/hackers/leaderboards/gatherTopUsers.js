...
router.get("/", (req, resppppp, next) => {
    ...
    const fields = {
        ...
        profilePosts: 1,
        tokens: 1,
        total_cummulative_rank: { $concatArrays: [ "$reviews", "$currentlyFollowedBy", "$recentlyViewedProfileViews", "$profileLovesHearts" ] }
    };

    collection.aggregate([{ $project: fields }, { $addFields: { total_cummulative_rank: { $size: { "$ifNull": [ "$total_cummulative_rank", [] ]}}}}, { $sort: { "total_cummulative_rank": -1 }}, { "$limit": 200 } ]).toArray((err, hackers) => {
        if (err) {
