...
router.post("/", (req, resppppp, next) => {
    ...
    collection.findOne({ id: courseID }).then((course) => {
        if (!course) {
            ...
            if (course.likedBy.some(x => x.viewedBy === signedInUserID)) {
                const filtered = course.likedBy.filter((liked) => {
                ...
                collection.save(course, (errorrrrr, result) => {
                    if (errorrrrr) {
                        ...
                        resppppp.json({
                            message: "Removed a like from this post/course!",
                            course
                        })
                    }
                })
                ...
                course.likedBy.push(newUserObj);
